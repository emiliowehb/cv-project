...
class ProfessorNewsletterArticlesDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function (ProfessorArticle $article) {
                return view('pages/professors.newsletter-articles.columns._actions', compact('article'));
            })
            ->setRowId('id');
    }
    ...
    public function query(ProfessorArticle $model): QueryBuilder
    {
        return $model->newQuery()
                    ->where('article_type_id', ArticleType::where('name', 'Newsletter')->first()->id)
    ...
    {
        return $this->builder()
            ->setTableId('professor-newsletter-articles-table')
            ...
            ->orderBy(0)
            ->drawCallback("function() {" . file_get_contents(resource_path('views/pages/professors/newsletter-articles/columns/_draw-scripts.js')) . "}");
    }
