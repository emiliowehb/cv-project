...
class ProfessorArticlesDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->rawColumns(['action', 'status'])
            ->editColumn('article_type_id', function (ProfessorArticle $article) {
    ...
    {
        return $this->builder()
            ->setTableId('professor-articles-table')
            ...
            ->orderBy(0)
            ->drawCallback("function() {" . file_get_contents(resource_path('views/pages/professors/articles/columns/_draw-scripts.js')) . "}");
    }

    public function getColumns(): array
    {
        return [
            Column::make('article_type_id')->title('Type')->addClass('text-start'),
