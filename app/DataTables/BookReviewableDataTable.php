...
class BookReviewableDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('name', function (Reviewable $reviewable) {
                return $reviewable->reviewable->name;
            })
            ...
            ->addColumn('actions', function (Reviewable $reviewable) {
                return view('pages/apps.professor-submissions.books.columns._actions', ['reviewable' => $reviewable]);
            })
            ->setRowId('id');
    }
    ...
    public function query(Reviewable $model): QueryBuilder
    {
        return $model->newQuery()
            ->where('reviewable_type', ProfessorBook::class)
            ->where('status', '=', ArticleStatusEnum::WAITING_FOR_VALIDATION)
            ->join('professor_books', 'reviewables.reviewable_id', '=', 'professor_books.id')
            ->join('professors', 'professor_books.professor_id', '=', 'professors.id')
            ->select('reviewables.*', 'professor_books.name', 'professor_books.year', 'professor_books.month', 'professors.first_name', 'professors.middle_name', 'professors.last_name');
    }
    ...
    {
        return $this->builder()
            ->setTableId('book-reviewables-table')
            ...
            ->orderBy(0)
            ->drawCallback("function() {" . file_get_contents(resource_path('views/pages/apps/professor-submissions/books/columns/_draw-scripts.js')) . "}");
    }
    ...
    {
        return 'BookReviewables_' . date('YmdHis');
    }
}
